...
public class Server{
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("Server running on " + InetAddress.getLocalHost() + ":" + serversocket.getLocalPort());
        final String CRLF = "\r\n";
        try {
            // boot up server
            ...
            if (!file.exists()) { // if requested file doesn't exist, response with 404
                ...
                writer.write(response); // send code
                // create header
                writer.append("Content-Disposition: attachment; filename=\"")
                        .append(file.getName())
                        .append("\"")
                        .append(CRLF);
                writer.append(CRLF).flush(); // send header
